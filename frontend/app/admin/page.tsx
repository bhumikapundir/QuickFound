...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab]   = useState<'all' | 'lost' | 'found' | 'claimed' | 'users'>('all')
  ...
  const loadItems = async () => {
    ...
    try {
      const filters: ItemFilters = {
        ...(activeTab !== 'all' && activeTab !== 'claimed' && activeTab !== 'users' && { type: activeTab as any }),
  ...
  useEffect(() => {
    if (user?.role === 'admin' && activeTab !== 'users') loadItems()
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/auth/reset-password', {
        ...
        body: JSON.stringify({ studentId: resetTarget.studentId, newPassword }),
      })
      const data = await res.json()
      if (data.success) {
        setResetSuccess(`Password reset successfully for ${resetTarget.name}`)
        setResetTarget(null)
        setNewPassword('')
      } else {
    ...
    <div style={{ background: 'var(--color-surface)', minHeight: '100dvh' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <p style={{ color: 'rgba(255,255,255,0.5)', margin: 0, fontSize: '0.875rem' }}>
                  Manage all campus items and students
                </p>
              </div>
            </div>
            <button
              onClick={() => activeTab === 'users' ? loadUsers() : loadItems()}
              className="qf-btn"
              style={{
      ...
      <div className="qf-container" style={{ paddingBlock: '2rem' }}>
        ...
        <div style={{
          ...
          {/* Search - only show for item tabs */}
          {activeTab !== 'users' && (
          ...
          <div>
            ...
            {resetSuccess && (
              <div style={{
                ...
                fontSize: '0.875rem', color: 'var(--color-found)',
            ...
            {usersLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="qf-skeleton" style={{ height: 68, borderRadius: 12 }} />
                ))}
              </div>
            ) : users.length === 0 ? (
              <EmptyState
                ...
                description="No student accounts exist in the system yet."
              />
            ) : (
              <>
                {/* Table header */}
                <div style={{
                  ...
                  {['Student', 'Student ID', 'Role', 'Actions'].map(h => (
                    <span key={h} style={{
                      fontSize: '0.72rem', fontWeight: 700,
                      textTransform: 'uppercase' as const,
                      letterSpacing: '0.07em',
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{
                          ...
                          background: u.role === 'admin' ? 'rgba(245,158,11,0.2)' : 'var(--color-surface-3)',
                          border: u.role === 'admin' ? '1px solid rgba(245,158,11,0.4)' : '1px solid var(--color-border)',
                          display: 'flex', alignItems: 'center', justifyContent: 'center',
                          fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem',
                          color: u.role === 'admin' ? 'var(--color-brand-amber)' : 'var(--color-text-secondary)',
                        }}>
                      ...
                      <span style={{
                        ...
                        fontSize: '0.72rem', fontWeight: 700,
                        background: u.role === 'admin' ? 'rgba(245,158,11,0.15)' : 'var(--color-surface-3)',
                        color: u.role === 'admin' ? 'var(--color-brand-amber)' : 'var(--color-text-muted)',
                        border: u.role === 'admin' ? '1px solid rgba(245,158,11,0.3)' : '1px solid var(--color-border)',
                        width: 'fit-content',
                      }}>
                      ...
                      <button
                        ...
                        onMouseEnter={e => {
                          e.currentTarget.style.background = 'var(--color-brand-amber)'
                ...
                <p style={{
                  fontSize: '0.8rem', color: 'var(--color-text-muted)',
                  marginTop: '0.75rem', textAlign: 'center' as const,
                }}>
                  {users.length} student{users.length !== 1 ? 's' : ''} in the system
                </p>
              </>
            )}
          </div>
      ...
      <Modal
        isOpen={!!resetTarget}
        onClose={() => { setResetTarget(null); setNewPassword(''); setResetError('') }}
        title="Reset Password"
        maxWidth={420}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontWeight: 700, margin: '0 0 2px', fontSize: '0.9rem', color: 'var(--color-text-primary)' }}>
                {resetTarget?.name}
              </p>
              <p style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)', margin: 0 }}>
                Student ID: {resetTarget?.studentId}
              </p>
            </div>
          </div>
          ...
          {resetError && (
            <div style={{
              ...
              background: 'var(--color-lost-bg)',
              border: '1px solid rgba(239,68,68,0.2)',
              fontSize: '0.85rem', color: 'var(--color-lost)',
            }}>
              ⚠️ {resetError}
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button
              ...
              style={{
                flex: 2,
                opacity: resetLoading || !newPassword || newPassword.length < 6 ? 0.6 : 1,
                cursor: resetLoading || !newPassword || newPassword.length < 6 ? 'not-allowed' : 'pointer',
              }}>
              {resetLoading ? (
